...
namespace KK_QuickAccessBox
{
    ...
    public partial class QuickAccessBox : BaseUnityPlugin
    {
        ...
        private static bool ItemMatchesSearch(ItemInfo item, string searchStr)
        {
            ...
            var splitChars = searchStr.Where(char.IsWhiteSpace).Concat(new[] { '/', '\\' }).ToArray();
            var splitSearchStr = searchStr.ToLowerInvariant().Split(splitChars, StringSplitOptions.RemoveEmptyEntries);
            return splitSearchStr.All(findStr => item.SearchStrings.Any(otherStr =>
                otherStr.Contains(findStr) ||
                findStr.Length <= otherStr.Length && CompareSimilarity(findStr, otherStr) < findStr.Length / 4));
        }
        /// <summary>
